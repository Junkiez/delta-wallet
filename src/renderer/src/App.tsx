...
export default function WalletPage() {
  return (
    <div className="relative h-screen py-6 w-full flex flex-col justify-between items-center bg-sky-950/70 overflow-hidden">
      <div />
      <Active />
      <div />
    </div>
...
function Active() {
  ...
  return (
    <div className=" text-white relative flex flex-col gap-4 items-center justify-center z-10">
      <div className="relative flex justify-center items-center w-36 h-36">
        <div className="relative w-36 h-36 flex items-center justify-center">
          <div className="absolute inset-0 rounded-full bg-radial from-sky-700/70 to-transparent blur-2xl"></div>
          <img src={iota} alt="iota" className="relative rounded-lg w-36" />
        </div>
      </div>
      <h1 className="text-3xl font-bold text-white">EtherDelta Wallet</h1>
      <p className="font-semibold text-lg">{progress}%</p>
      <div className="flex justify-start rounded-full h-1 w-96 bg-white/10">
        <div
          style={{ width: `${progress}%` }}
          className="flex justify-start rounded-full h-1 bg-gradient-to-r from-sky-500 to-sky-300 shadow-lg"
        />
      </div>
      <div className="text-center flex flex-col items-center text-gray-400 space-y-4">
        <p className="text-sm text-gray-400">{currentHeader}</p>
        <div className="flex gap-2">
          <p className="h-2 w-2 bg-gray-300 rounded-full animate-pulse delay-0 duration-300"></p>
          <p className="h-2 w-2 bg-gray-300 rounded-full animate-pulse delay-100 duration-300"></p>
          <p className="h-2 w-2 bg-gray-300 rounded-full animate-pulse delay-200 duration-300"></p>
        </div>
      </div>
    </div>
  )
